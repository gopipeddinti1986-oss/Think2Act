...
from sqlalchemy import Column, String, DateTime, Text, ForeignKey, Boolean, Index, Uuid, JSON
...
class AIAction(Base):
    ...
    id = Column(Uuid, primary_key=True, default=uuid.uuid4)
    user_id = Column(Uuid, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    conversation_id = Column(Uuid, ForeignKey("ai_conversations.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    payload = Column(JSON, nullable=True)              # Parameters proposed by AI (compatible across PostgreSQL/SQLite)
